  ...
  saveImage = AsyncHandler(async (req: Request, res: Response, next: NextFunction) => {
    if (req.files) {
      if ((req.files as any).cover) {
        ...
        await sharp((req.files as any).cover[0].buffer).webp({ quality: 99 }).resize(1920, 1080).toFile(`uploads/images/products/${fileName}`)
        req.body.cover = fileName;
      }
      if ((req.files as any).images) {
        req.body.images = []
        await Promise.all((req.files as any).images.map(async (image: any, index: number) => {
